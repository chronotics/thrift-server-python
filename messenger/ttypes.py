#
# Autogenerated by Thrift Compiler (0.12.0)
#
# DO NOT EDIT UNLESS YOU ARE SURE THAT YOU KNOW WHAT YOU ARE DOING
#
#  options string: py
#

from thrift.Thrift import TType, TMessageType, TFrozenDict, TException, TApplicationException
from thrift.protocol.TProtocol import TProtocolException
from thrift.TRecursive import fix_spec

import sys

from thrift.transport import TTransport
all_structs = []


class Message(object):
    """
    Attributes:
     - _sender_id
     - _receiver_id
     - _subject
     - _sequence_no
     - _timestamp
     - _list_bool
     - _list_i16
     - _list_i32
     - _list_i64
     - _list_double
     - _list_string
     - _binary
     - _payload

    """


    def __init__(self, _sender_id=None, _receiver_id=None, _subject=None, _sequence_no=None, _timestamp=None, _list_bool=None, _list_i16=None, _list_i32=None, _list_i64=None, _list_double=None, _list_string=None, _binary=None, _payload=None,):
        self._sender_id = _sender_id
        self._receiver_id = _receiver_id
        self._subject = _subject
        self._sequence_no = _sequence_no
        self._timestamp = _timestamp
        self._list_bool = _list_bool
        self._list_i16 = _list_i16
        self._list_i32 = _list_i32
        self._list_i64 = _list_i64
        self._list_double = _list_double
        self._list_string = _list_string
        self._binary = _binary
        self._payload = _payload

    def read(self, iprot):
        if iprot._fast_decode is not None and isinstance(iprot.trans, TTransport.CReadableTransport) and self.thrift_spec is not None:
            iprot._fast_decode(self, iprot, [self.__class__, self.thrift_spec])
            return
        iprot.readStructBegin()
        while True:
            (fname, ftype, fid) = iprot.readFieldBegin()
            if ftype == TType.STOP:
                break
            if fid == 1:
                if ftype == TType.STRING:
                    self._sender_id = iprot.readString().decode('utf-8') if sys.version_info[0] == 2 else iprot.readString()
                else:
                    iprot.skip(ftype)
            elif fid == 2:
                if ftype == TType.STRING:
                    self._receiver_id = iprot.readString().decode('utf-8') if sys.version_info[0] == 2 else iprot.readString()
                else:
                    iprot.skip(ftype)
            elif fid == 3:
                if ftype == TType.STRING:
                    self._subject = iprot.readString().decode('utf-8') if sys.version_info[0] == 2 else iprot.readString()
                else:
                    iprot.skip(ftype)
            elif fid == 4:
                if ftype == TType.STRING:
                    self._sequence_no = iprot.readString().decode('utf-8') if sys.version_info[0] == 2 else iprot.readString()
                else:
                    iprot.skip(ftype)
            elif fid == 5:
                if ftype == TType.STRING:
                    self._timestamp = iprot.readString().decode('utf-8') if sys.version_info[0] == 2 else iprot.readString()
                else:
                    iprot.skip(ftype)
            elif fid == 6:
                if ftype == TType.LIST:
                    self._list_bool = []
                    (_etype3, _size0) = iprot.readListBegin()
                    for _i4 in range(_size0):
                        _elem5 = iprot.readBool()
                        self._list_bool.append(_elem5)
                    iprot.readListEnd()
                else:
                    iprot.skip(ftype)
            elif fid == 7:
                if ftype == TType.LIST:
                    self._list_i16 = []
                    (_etype9, _size6) = iprot.readListBegin()
                    for _i10 in range(_size6):
                        _elem11 = iprot.readI16()
                        self._list_i16.append(_elem11)
                    iprot.readListEnd()
                else:
                    iprot.skip(ftype)
            elif fid == 8:
                if ftype == TType.LIST:
                    self._list_i32 = []
                    (_etype15, _size12) = iprot.readListBegin()
                    for _i16 in range(_size12):
                        _elem17 = iprot.readI32()
                        self._list_i32.append(_elem17)
                    iprot.readListEnd()
                else:
                    iprot.skip(ftype)
            elif fid == 9:
                if ftype == TType.LIST:
                    self._list_i64 = []
                    (_etype21, _size18) = iprot.readListBegin()
                    for _i22 in range(_size18):
                        _elem23 = iprot.readI64()
                        self._list_i64.append(_elem23)
                    iprot.readListEnd()
                else:
                    iprot.skip(ftype)
            elif fid == 10:
                if ftype == TType.LIST:
                    self._list_double = []
                    (_etype27, _size24) = iprot.readListBegin()
                    for _i28 in range(_size24):
                        _elem29 = iprot.readDouble()
                        self._list_double.append(_elem29)
                    iprot.readListEnd()
                else:
                    iprot.skip(ftype)
            elif fid == 11:
                if ftype == TType.LIST:
                    self._list_string = []
                    (_etype33, _size30) = iprot.readListBegin()
                    for _i34 in range(_size30):
                        _elem35 = iprot.readString().decode('utf-8') if sys.version_info[0] == 2 else iprot.readString()
                        self._list_string.append(_elem35)
                    iprot.readListEnd()
                else:
                    iprot.skip(ftype)
            elif fid == 12:
                if ftype == TType.STRING:
                    self._binary = iprot.readBinary()
                else:
                    iprot.skip(ftype)
            elif fid == 13:
                if ftype == TType.STRING:
                    self._payload = iprot.readString().decode('utf-8') if sys.version_info[0] == 2 else iprot.readString()
                else:
                    iprot.skip(ftype)
            else:
                iprot.skip(ftype)
            iprot.readFieldEnd()
        iprot.readStructEnd()

    def write(self, oprot):
        if oprot._fast_encode is not None and self.thrift_spec is not None:
            oprot.trans.write(oprot._fast_encode(self, [self.__class__, self.thrift_spec]))
            return
        oprot.writeStructBegin('Message')
        if self._sender_id is not None:
            oprot.writeFieldBegin('_sender_id', TType.STRING, 1)
            oprot.writeString(self._sender_id.encode('utf-8') if sys.version_info[0] == 2 else self._sender_id)
            oprot.writeFieldEnd()
        if self._receiver_id is not None:
            oprot.writeFieldBegin('_receiver_id', TType.STRING, 2)
            oprot.writeString(self._receiver_id.encode('utf-8') if sys.version_info[0] == 2 else self._receiver_id)
            oprot.writeFieldEnd()
        if self._subject is not None:
            oprot.writeFieldBegin('_subject', TType.STRING, 3)
            oprot.writeString(self._subject.encode('utf-8') if sys.version_info[0] == 2 else self._subject)
            oprot.writeFieldEnd()
        if self._sequence_no is not None:
            oprot.writeFieldBegin('_sequence_no', TType.STRING, 4)
            oprot.writeString(self._sequence_no.encode('utf-8') if sys.version_info[0] == 2 else self._sequence_no)
            oprot.writeFieldEnd()
        if self._timestamp is not None:
            oprot.writeFieldBegin('_timestamp', TType.STRING, 5)
            oprot.writeString(self._timestamp.encode('utf-8') if sys.version_info[0] == 2 else self._timestamp)
            oprot.writeFieldEnd()
        if self._list_bool is not None:
            oprot.writeFieldBegin('_list_bool', TType.LIST, 6)
            oprot.writeListBegin(TType.BOOL, len(self._list_bool))
            for iter36 in self._list_bool:
                oprot.writeBool(iter36)
            oprot.writeListEnd()
            oprot.writeFieldEnd()
        if self._list_i16 is not None:
            oprot.writeFieldBegin('_list_i16', TType.LIST, 7)
            oprot.writeListBegin(TType.I16, len(self._list_i16))
            for iter37 in self._list_i16:
                oprot.writeI16(iter37)
            oprot.writeListEnd()
            oprot.writeFieldEnd()
        if self._list_i32 is not None:
            oprot.writeFieldBegin('_list_i32', TType.LIST, 8)
            oprot.writeListBegin(TType.I32, len(self._list_i32))
            for iter38 in self._list_i32:
                oprot.writeI32(iter38)
            oprot.writeListEnd()
            oprot.writeFieldEnd()
        if self._list_i64 is not None:
            oprot.writeFieldBegin('_list_i64', TType.LIST, 9)
            oprot.writeListBegin(TType.I64, len(self._list_i64))
            for iter39 in self._list_i64:
                oprot.writeI64(iter39)
            oprot.writeListEnd()
            oprot.writeFieldEnd()
        if self._list_double is not None:
            oprot.writeFieldBegin('_list_double', TType.LIST, 10)
            oprot.writeListBegin(TType.DOUBLE, len(self._list_double))
            for iter40 in self._list_double:
                oprot.writeDouble(iter40)
            oprot.writeListEnd()
            oprot.writeFieldEnd()
        if self._list_string is not None:
            oprot.writeFieldBegin('_list_string', TType.LIST, 11)
            oprot.writeListBegin(TType.STRING, len(self._list_string))
            for iter41 in self._list_string:
                oprot.writeString(iter41.encode('utf-8') if sys.version_info[0] == 2 else iter41)
            oprot.writeListEnd()
            oprot.writeFieldEnd()
        if self._binary is not None:
            oprot.writeFieldBegin('_binary', TType.STRING, 12)
            oprot.writeBinary(self._binary)
            oprot.writeFieldEnd()
        if self._payload is not None:
            oprot.writeFieldBegin('_payload', TType.STRING, 13)
            oprot.writeString(self._payload.encode('utf-8') if sys.version_info[0] == 2 else self._payload)
            oprot.writeFieldEnd()
        oprot.writeFieldStop()
        oprot.writeStructEnd()

    def validate(self):
        if self._sender_id is None:
            raise TProtocolException(message='Required field _sender_id is unset!')
        if self._receiver_id is None:
            raise TProtocolException(message='Required field _receiver_id is unset!')
        if self._subject is None:
            raise TProtocolException(message='Required field _subject is unset!')
        if self._sequence_no is None:
            raise TProtocolException(message='Required field _sequence_no is unset!')
        if self._timestamp is None:
            raise TProtocolException(message='Required field _timestamp is unset!')
        if self._list_bool is None:
            raise TProtocolException(message='Required field _list_bool is unset!')
        if self._list_i16 is None:
            raise TProtocolException(message='Required field _list_i16 is unset!')
        if self._list_i32 is None:
            raise TProtocolException(message='Required field _list_i32 is unset!')
        if self._list_i64 is None:
            raise TProtocolException(message='Required field _list_i64 is unset!')
        if self._list_double is None:
            raise TProtocolException(message='Required field _list_double is unset!')
        if self._list_string is None:
            raise TProtocolException(message='Required field _list_string is unset!')
        if self._binary is None:
            raise TProtocolException(message='Required field _binary is unset!')
        return

    def __repr__(self):
        L = ['%s=%r' % (key, value)
             for key, value in self.__dict__.items()]
        return '%s(%s)' % (self.__class__.__name__, ', '.join(L))

    def __eq__(self, other):
        return isinstance(other, self.__class__) and self.__dict__ == other.__dict__

    def __ne__(self, other):
        return not (self == other)
all_structs.append(Message)
Message.thrift_spec = (
    None,  # 0
    (1, TType.STRING, '_sender_id', 'UTF8', None, ),  # 1
    (2, TType.STRING, '_receiver_id', 'UTF8', None, ),  # 2
    (3, TType.STRING, '_subject', 'UTF8', None, ),  # 3
    (4, TType.STRING, '_sequence_no', 'UTF8', None, ),  # 4
    (5, TType.STRING, '_timestamp', 'UTF8', None, ),  # 5
    (6, TType.LIST, '_list_bool', (TType.BOOL, None, False), None, ),  # 6
    (7, TType.LIST, '_list_i16', (TType.I16, None, False), None, ),  # 7
    (8, TType.LIST, '_list_i32', (TType.I32, None, False), None, ),  # 8
    (9, TType.LIST, '_list_i64', (TType.I64, None, False), None, ),  # 9
    (10, TType.LIST, '_list_double', (TType.DOUBLE, None, False), None, ),  # 10
    (11, TType.LIST, '_list_string', (TType.STRING, 'UTF8', False), None, ),  # 11
    (12, TType.STRING, '_binary', 'BINARY', None, ),  # 12
    (13, TType.STRING, '_payload', 'UTF8', None, ),  # 13
)
fix_spec(all_structs)
del all_structs
